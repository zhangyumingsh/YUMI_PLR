...
#SD_PATH=~/gcode_files
#cat ${2} > /tmp/plrtmpA.$$
mkdir -p /home/biqu/printer_data/gcodes/plr/
filepath=$(sed -n "s/.*filepath *= *'\([^']*\)'.*/\1/p" /home/biqu/printer_data/config/variables.cfg)
filepath=$(printf "$filepath")
echo "$filepath"
#SD_PATH=$(dirname "$filepath")
last_file=$(sed -n "s/.*last_file *= *'\([^']*\)'.*/\1/p" /home/biqu/printer_data/config/variables.cfg)
last_file=$(printf "$last_file")
echo "$last_file"
plr=$last_file
echo "plr=$plr" 
PLR_PATH=/home/biqu/printer_data/gcodes/plr/
#echo "$SD_PATH"
#SD_PATH=~/gcode_files
#cat ${SD_PATH}/${2} > /home/biqu/plrtmpA.$$
cat "${filepath}" > /home/biqu/plrtmpA.$$
cat /home/biqu/plrtmpA.$$ | sed -e '1,/Z'${1}'/ d' | sed -ne '/ Z/,$ p' | grep -m 1 ' Z' | sed -ne 's/.* Z\([^ ]*\).*/SET_KINEMATIC_POSITION Z=\1/p' > ${PLR_PATH}/"${plr}"
#echo 'START_TEMPS' >> ${SD_PATH}/plr.gcode
echo 'M118 START_TEMPS...' >> ${PLR_PATH}/"${plr}"
cat /home/biqu/plrtmpA.$$ | sed '/ Z'${1}'/q' | sed -ne '/\(M104\|M140\|M109\|M190\|M106\)/p' >> ${PLR_PATH}/"${plr}"
cat /home/biqu/plrtmpA.$$ | sed -ne '/;End of Gcode/,$ p' | tr '\n' ' ' | sed -ne 's/ ;[^ ]* //gp' | sed -ne 's/\\\\n/;/gp' | tr ';' '\n' | grep material_bed_temperature | sed -ne 's/.* = /M140 S/p' | head -1 >> ${PLR_PATH}/"${plr}"
...
BG_EX=`tac /home/biqu/plrtmpA.$$ | sed -e '/ Z'${1}'[^0-9]*$/q' | tac | tail -n+2 | sed -e '/ Z[0-9]/ q' | tac | sed -e '/ E[0-9]/ q' | sed -ne 's/.* E\([^ ]*\)/G92 E\1/p'`
...
if [ "${BG_EX}" = "" ]; then
 BG_EX=`tac /home/biqu/plrtmpA.$$ | sed -e '/ Z'${1}'[^0-9]*$/q' | tac | tail -n+2 | sed -ne '/ Z/,$ p' | sed -e '/ E[0-9]/ q' | sed -ne 's/.* E\([^ ]*\)/G92 E\1/p'`
fi
M83=$(cat /home/biqu/plrtmpA.$$ | sed '/ Z'${1}'/q' | sed -ne '/\(M83\)/p')
if [ -n "${M83}" ];then
...
tac /home/biqu/plrtmpA.$$ | sed -e '/ Z'${1}'[^0-9]*$/q' | tac | tail -n+2 | sed -ne '/ Z/,$ p' >> ${PLR_PATH}/"${plr}"
rm /home/biqu/plrtmpA.$$
