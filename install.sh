...
if [ -n "$SUDO_USER" ]; then
    ...
    if [ "$SUDO_USER" = "runner" ]; then
        # Définir USER_HOME spécifiquement pour 'runner' et définir OWNER à 'biqu'
        USER_HOME="/home/biqu"
        OWNER="biqu"
    else
...
if [ "$1" == "remove" ]; then
  ...
  cp -f $PROJECT_DIR/plr.cfg $USER_HOME/printer_data/config/ && echo "plr.cfg cobiqued successfully." || echo "Error copying plr.cfg."
  cp -f $PROJECT_DIR/gcode_shell_command.py $KLIPPER_DIR/klippy/extras/ && echo "gcode_shell_command.py cobiqued successfully." || echo "Error copying gcode_shell_command.py."
...
if [ -n "$SUDO_USER" ]; then
    ...
    # Exécuter la commande chown avec les droits de l'utilisateur spécifique (biqu:biqu pour runner, sinon SUDO_USER)
